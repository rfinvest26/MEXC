...
function assertServerOnly() {
  ...
  const hasWindow = typeof globalThis !== 'undefined' && 'window' in (globalThis as unknown as object);
  if (hasWindow) {
...
export function createSupabaseAdminClient(): SupabaseClient {
  ...
  if (!url) throw new Error('Missing required env var: SUPABASE_URL (or VITE_SUPABASE_URL for server)');
